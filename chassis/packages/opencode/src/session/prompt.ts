    ...
    const shellImpl = Effect.fn("SessionPrompt.shellImpl")(function* (input: ShellInput, ready?: Latch.Latch) {
      return yield* Effect.uninterruptibleMask((restore) =>
        Effect.gen(function* () {
          ...
          const { msg, part, cwd, agent, session } = yield* Effect.gen(function* () {
            const ctx = yield* InstanceState.context
            ...
            return { msg, part, cwd: ctx.directory, agent, session }
          ...
          // Honor permission.bash (Artemis sets deny) — shell mode previously bypassed it.
          const bashRule = Permission.evaluate(
            "bash",
            input.command,
