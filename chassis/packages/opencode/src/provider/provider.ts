  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        // extend database from config FIRST so custom providers (e.g. cursor) exist
        // before plugin model discovery runs
        ...
        // Plugin model discovery AFTER config database + auth so Cursor official catalog can load
        for (const hook of plugins) {
          ...
          // Keep catalog on already-connected providers in sync
          if (providers[providerID]) {
            providers[providerID].models = provider.models
          }
        }
        ...
        for (const [id, provider] of configProviders) {
          const providerID = ProviderV2.ID.make(id)
          // Artemis: config alone must NOT mark a provider connected / selectable for chat.
          // Only refresh metadata when already authenticated (env/api/oauth).
          if (process.env.ARTEMIS === "1" && !providers[providerID]) {
            continue
          }
          ...
          // Preserve real auth source when already connected
          if (providers[providerID]?.source && providers[providerID].source !== "config") {
            delete (partial as { source?: string }).source
          }
