...
export type PromptProps = {
  ...
  visible?: boolean
  disabled?: boolean
  /** Return false to cancel submit (e.g. restart confirm). */
...
export function Prompt(props: PromptProps) {
  ...
  const showAgentSwitcher = createMemo(() => {
    if (process.env.ARTEMIS === "1") return false
    return local.agent.list().length > 1
  })
  ...
  const currentProviderLabel = createMemo(() => {
    // Product chrome: model name only — provider is visible in /connect / model picker.
  ...
  const usage = createMemo(() => {
    // Artemis: never show OpenCode stub chat tokens (e.g. "72 (0%)").
    // Real swarm usage lives in the sidebar Context panel via daemon usage_update.
    if (process.env.ARTEMIS === "1") return
  ...
  const promptCommands = createMemo(() =>
    [
      ...
      .filter((entry) => {
        ...
        // Drop non-CTF slash noise from the prompt palette.
        const dropSlash = new Set(["editor", "skills", "warp", "move"])
        const slash = "slashName" in entry ? entry.slashName : undefined
        if (typeof slash === "string" && dropSlash.has(slash)) return false
        return true
      })
      .map((entry) => ({
      namespace: "palette",
  ...
  const placeholderText = createMemo(() => {
    ...
    if (process.env.ARTEMIS === "1") {
      return "Paste a challenge, path, or @files…"
    }
