...
function init() {
  ...
  return {
    ...
    /** Push a dialog on top without dismissing the one underneath. */
    ...
    /** Pop the top dialog only. When ``invokeClose`` is false, skip its onClose. */
    pop(opts?: { invokeClose?: boolean }) {
      if (store.stack.length === 0) return
      const current = store.stack.at(-1)
      if (opts?.invokeClose !== false) current?.onClose?.()
      setStore("stack", store.stack.slice(0, -1))
      if (store.stack.length === 0) {
        setStore("size", "medium")
      }
