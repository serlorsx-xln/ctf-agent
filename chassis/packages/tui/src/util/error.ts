...
export function cliErrorMessage(input: unknown): string | undefined {
  ...
  if (model) {
    ...
    return [
      ...
      "Try picking another model from the model list",
      "Or reconnect the provider with /connect",
    ].join("\n")
  }
  ...
  if (isRecord(input) && named(input, "MCPFailed")) {
    ...
    return `MCP server "${name}" failed. Note, Artemis does not support MCP authentication yet.`
  }
  return undefined
}
