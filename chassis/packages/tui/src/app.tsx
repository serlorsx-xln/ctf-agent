...
import { DialogModel, prefillModelFromRaceSpec } from "./component/dialog-model"
import { daemon } from "./artemis/client"
import { hasTerminalSolveOutcome } from "./util/artemis-live-log"
...
const appBindingCommandsArtemis = [
  "command.palette.show",
  "model.list",
  "provider.connect",
  "opencode.status",
...
export const run = Effect.fn("Tui.run")(function* (input: TuiInput) {
  ...
  const result = yield* Effect.scoped(
    Effect.gen(function* () {
      ...
      // Artemis: /exit · Ctrl+C · process teardown must stop the detached swarm
      // so sandbox containers do not keep burning CPU/disk after the TUI is gone.
          ...
          return (
            <ExitProvider
              exit={(reason) => {
                ...
                if (process.env.ARTEMIS === "1") {
                  void daemon.shutdown().finally(finish)
                  return
                }
                finish()
              }}
...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
  ...
  createEffect(() => {
    ...
    if (route.data.type === "home") {
      renderer.setTerminalTitle("Artemis")
      return
    }
    ...
    if (route.data.type === "session") {
      ...
      renderer.setTerminalTitle(`A | ${title}`)
      return
    }

    if (route.data.type === "plugin") {
      renderer.setTerminalTitle(`A | ${route.data.id}`)
    }
  })
  ...
  onMount(() => {
    if (process.env.ARTEMIS === "1") {
      // Do not wipe a finished solve the operator is still reading — only clear
      // when there is no terminal outcome / completed flow yet.
      const multi = daemon.swarmRoster[0]().length > 1 || daemon.lastModels[0]().length > 1
      if (
        !daemon.flowCompleted[0]() &&
        !hasTerminalSolveOutcome(daemon.swarmEvents[0](), multi)
      ) {
  ...
  // Keep Artemis daemon peer bound to the active OpenCode chat session.
  createEffect(() => {
  ...
  createEffect(
    on(
      () => {
        ...
        // Artemis: open /connect until any provider has real credentials (not config-only)
        if (process.env.ARTEMIS === "1") {
  ...
  const appCommands = createMemo(() =>
    [
      ...
      })),
      {
        ...
        run: () => {
          if (process.env.ARTEMIS === "1") {
            ...
            prefillModelFromRaceSpec(cursorSpec ?? models[0], local)
          }
      ...
      },
      {
        ...
        category: "Agent",
        slashName: "agents",
        hidden: process.env.ARTEMIS === "1" || local.agent.list().length <= 1,
        run: () => {
      ...
      },
      {
        ...
        title: "Open docs",
        run: () => {
          // Prefer local README — do not surface third-party brand URLs in-product.
          const root = process.env.ARTEMIS_REPO_ROOT || process.cwd()
          open(`file://${root}/README.md`).catch(() => {})
          dialog.clear()
        },
        category: "System",
      },
      ...
      .filter((command) => {
        ...
        // Artemis product surface: only CTF-relevant slash/commands.
        const keep = new Set([
          "command.palette.show",
          "session.new",
          "provider.connect",
        ...
        if (keep.has(command.name)) return true
        // Keep hidden keybind-only helpers that don't expose slash names.
        if (command.hidden === true && !("slashName" in command && command.slashName)) return true
        return false
      })
      .map((command) => ({
        namespace: "palette",
  ...
  useBindings(() => ({
    ...
    bindings: tuiConfig.keybinds.gather(
      "app",
      process.env.ARTEMIS === "1" ? appBindingCommandsArtemis : appBindingCommands,
    ),
  }))
  ...
  event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to Artemis v${result.data.version}. Please restart the application.`,
    )
