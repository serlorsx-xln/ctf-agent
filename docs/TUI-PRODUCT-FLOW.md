...
2. **Before TUI:** the launcher runs a **detailed** inventory (Docker, L0, full pack caches, qemu guest `libstdc++` / `libgcc_s` on pwn+mobile). Missing L0, pack caches, or guest libs **automatically runs full setup** (live logs; TUI stays closed until done). Already-complete inventory is a no-op. Warm images stay optional. `ARTEMIS_SKIP_LAUNCH_SETUP=1` skips the gate entirely; `ARTEMIS_SETUP_AUTO=0` skips setup (CI / non-interactive opt-out); `ARTEMIS_SETUP_AUTO=1` is an explicit force (same as the default when incomplete). If Docker/L0 is still missing after skip/fail, the in-TUI **Install sandbox** dialog opens and blocks load/solve until Install finishes (`setup_status` / `setup_install`). In-TUI Install builds **L0 only** (`packs=[]`, `skip_warm_runtime=true`); packs attach on demand. Full bake / guest-lib rebuild: `artemis setup` or relaunch (auto full setup).
...
4. Drop challenge flexibly in one message:
   ...
   - **One or many folders** and/or **one or many files** from **any user path** (Downloads, Desktop, `/tmp`, `/Volumes`, … → challenge root or materialized workspace + `distfiles/`). System trees and secret dirs (`.ssh`, `/etc`) stay denied.
